...
export type DemoAppointment = {
  ...
  canal: "web" | "whatsapp" | "instagram" | "recepcion";
  nota?: string;
};
...
export const upcomingBirthdays = demoClients.filter((client) => client.birthday >= "2026-03-23" && client.birthday <= "2026-04-06");

export const inactiveClients = demoClients.filter((client) => {
  const lastVisit = new Date(`${client.ultimaVisita}T00:00:00Z`).getTime();
  const today = new Date("2026-03-23T00:00:00Z").getTime();
